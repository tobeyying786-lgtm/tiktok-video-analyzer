/**
 * tab1-shots.js — V3.5.4: 去emoji
 */

function renderTab1(shots) {
  const tc = {};
  shots.forEach(s => { const t = s.shot_type || '其他'; tc[t] = (tc[t] || 0) + 1; });
  ...
  Object.entries(tc).sort((a, b) => b[1] - a[1]).forEach(([t, n]) => {
    stats += '<div class="ss-item"><div class="ssn">' + n + '</div><div class="ssl"><span class="bg-' + shotCls(t) + '">' + esc(t) + '</span></div></div>';
  });

  document.getElementById('t1').innerHTML =
    '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">' +
      '<div style="font-size:15px;font-weight:700">全部镜头（' + shots.length + '个分镜）</div>' +
      '<div style="font-size:12px;color:var(--text3)">点击镜头可放大查看 | 橙框=产品首现</div>' +
    '</div>' +
